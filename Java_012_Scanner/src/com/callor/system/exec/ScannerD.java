...
public class ScannerD {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			/*
			 * strNum1에 저장된 문자열이 QUIT가 아니고, 문자열형 정수값이 아닌경우
			 * 1. 그냥 Enter를 누른경우 ""
			 * 2. 정수가 아닌 실수 값을 입력한 경우 30.0 등
			 * 3. space(빈칸)을 입력한 경우 "30 ", " 30" ...
			 * 4. 그 외...
			 */
			...
			try {
				...
				/*
				 * try - catch 블럭에서 exception이 발생한 경우
				 * Integer.valueOf(strNum1) 명령이 실행되는 도중
				 * 정상적으로 코드가 작동되지 않았을때 catch{} 내의 코드가 실행된다
				 * try - catch 블럭으로 감싼 코드는 exception이 발생하여도 코드가 중단되는 것을 일단 방지한다
				 */
				// TODO: handle exception
			}
