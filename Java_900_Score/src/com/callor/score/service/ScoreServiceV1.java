...
/*
 * Java를 이용한 Application 에서 일반적인 패턴으로 Service 클래스를 사용한다
 * Service 클래스는 역할이 주로 비즈니스로직 코드가 작성된다
 * 직접적으로 사용자에게 하는일이 노출되지 않고, 데이터 연산, 가공, 통계 등을 수행하는 코드이다
 * 
...
public class ScoreServiceV1 {
	...
	/*
	 * scoreList 객체 변수의 데이터를 전달받아 변수값을 초기화(setting)하는 method
	 *  1. ScoreDto 클래스를 배열로 하는type의 scoreList 매개변수 선언
	 *  2. 다른곳에서 전달받은 scoreList 변수의 값을 this.scoreList 변수에할당하기
	 * 
	 */
	 ...
	 * 이 method 는 같은 클래스 scope 의 method 들만 사용할수 있도록 하는 것
	 */
...
//		int intArt = 0;
		...
		for (int i = 0; i < scoreList.length; i++) {
			...
			// 평균의 평균을 출력하기 위한 준비
			// 각 학생의 평균의 총 합산을 계산하여 floatAvg에 저장해두기
