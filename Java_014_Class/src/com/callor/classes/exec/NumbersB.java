...
public class NumbersB {
	...
	public static void main(String[] args) {
		...
		/*
		 * 문자열형 숫자를 4칙연산이 되는 숫자 데이터로 변환시키는 방법에는 2가지 method가 있다
		 * Interger.parseInt(), Float.parseFloat()
		 * 		문자열형 숫자를 primitive type의 숫자로 변환한다
		 * Integer.valueOf(), Float,valueOf()
		 *  	문자열형 숫자를 참조형숫자로 변환한다
		 *  	만약 변수의 type이 primitive 인경우
		 *  	이 method는 데이터를 Unboxing 하는 절차가 추가된다
		 *  	일부에서는 성능상 Issue 가 있다곤 하지만 지금은 크게 문제되지않는다
		 *  
		 *  문자열형 숫자를 실제 숫자로 바꿀때는 Type.valueOf() method를 사용해도 큰 문제가 없다
		 */
